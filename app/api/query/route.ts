...
export async function POST(req: Request) {
  ...
  const [meals, events, timetable] = await Promise.all([loadMeals(), loadEvents(), loadTimetable()]);
  ...
  // ---------- 일정 처리 ----------
  if (q.includes("일정") || q.includes("행사") || q.includes("event") || q.includes("exam") || q.includes("시험")) {
